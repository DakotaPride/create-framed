...
public class DyedFrogportVisual extends AbstractBlockEntityVisual<DyedFrogportBlockEntity> implements SimpleDynamicVisual {
    ...
    public DyedFrogportVisual(VisualizationContext ctx, DyedFrogportBlockEntity blockEntity, float partialTick,
    ...
    public void updateGoggles() {
        if (blockEntity.goggles && !lastGoggles) {
            ...
            head = instancerProvider()
                    .instancer(InstanceTypes.TRANSFORMED, Models.partial(head_gogglesModel))
                    .createInstance();
            lastHeadPitch = -1;
            updateLight(0);
            lastGoggles = true;
